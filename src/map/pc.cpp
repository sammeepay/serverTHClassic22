...
bool pc_steal_item(map_session_data *sd,block_list *bl, uint16 skill_lv)
{
	...
	if (md->master_id || status_has_mode(md_status, MD_STATUSIMMUNE) || util::vector_exists(status_get_race2(md), RC2_TREASURE) ||
		map_getmapflag(bl->m, MF_NOMOBLOOT) || md->get_bosstype() != BOSSTYPE_MVP && map_getmapflag(bl->m, MF_NOLOOTNORMALMOB) || // check noloot map flag. 
		(battle_config.skill_steal_max_tries && //Reached limit of steal attempts.
